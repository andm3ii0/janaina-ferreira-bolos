...
function FinalizarPedido() {
  const aplyButtonClick = (
    ...
    if (codigo.length !== 7 || codigo[0] !== 'T' || isNaN(parseInt(codigo[1]+codigo[2])) || (codigo[3]+codigo[4] !== 'MR' || isNaN(parseInt(codigo[5]+codigo[6])))) {
  ...
  const concluirButtonClick = (codigo, nome, tamanho, sabor) => {
    ...
    if (localStorage.getItem('pedidos') === null) {
      console.log('oi');
      localStorage.setItem('pedidos', JSON.stringify([{codigo, nome, tamanho, sabor}]))
    } else {
      console.log('oi2');
      localStorage.setItem('pedidos', JSON.stringify([...JSON.parse(localStorage.getItem('pedidos')), {codigo, nome, tamanho, sabor}]))
    }
  }
  return (
    <Context.Consumer>
        ...
        return (
          <div>
            ...
            <form>
              ...
              <button type="button" onClick={ () => concluirButtonClick(codigo, nome, tamanho, sabor)}>Concluir</button>
            </form>
          </div>
        );
      }}
